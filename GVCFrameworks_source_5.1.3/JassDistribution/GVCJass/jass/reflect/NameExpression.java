...
public class NameExpression extends Expression
{
    ...
    public void addChildren(String ref, Set locals)
    {
        ...
        Expression[] help = new Expression[children.length + st.countTokens()];
    ...
    public void insertMethodCall(ArgumentExpression ae)
    {
        children[children.length - 1] =
            new MethodCallExpression(
                ((SimpleNameExpression)children[children.length - 1]).getName(),
                ae);
        ...
        else
        {
            children[children.length - 1] =
                new ArrayAccessExpression(
                    ((Expression)children[children.length - 1]),
                    e);
     ...
     * @see
     */
    public String toStringWithChangedContext(
        Method m_old,
        Method m_new,
        String ref,
        boolean referenced)
    {
        ...
        for (int i = 0; i < children.length; i++)
        {
            if (!first)
            {
                sb.append(".");
                sb.append(
                    children[i].toStringWithChangedContext(
                        m_old,
                        m_new,
                        ref,
                        true));
            ...
            else
            {
                ...
                if (children[i] instanceof SimpleNameExpression
                    && ((SimpleNameExpression)children[i]).getName().equals(
                        "this"))
                {
                    sb.append(ref);
                }
                ...
                {
                    if (children[i] instanceof SimpleNameExpression
                        && (((SimpleNameExpression)children[i])
                            .getName()
                            .equals("Old")
                            || ((SimpleNameExpression)children[i])
                                .getName()
    ...
    public void reflectExpression(Context c, DependencyCollector dc)
    {
        ...
        if (children[0] instanceof SimpleNameExpression)
        {
            ...
            String classname =
                NameAnalysis.expandTypeName(children[0].toString(), container);
            ...
            else
            {
                ...
                {
                    int loop;
                    StringBuffer sb = new StringBuffer(children[0].toString());

                    for (loop = 1; loop < children.length; loop++)
                    {
                        if (!(children[loop] instanceof SimpleNameExpression))
                            break;
                        String testSb = sb.toString() + "." + children[loop];
                        if ((NameAnalysis.lookUpInClassPath(sb.toString()))
                            && (!NameAnalysis.lookUpInClassPath(testSb)))
                            break;
                        sb.append(".");
        ...
        for (int i = start_loop; i < children.length; i++)
        {
            ...
            {
                ((MethodCallExpression)children[i]).reflectExpression(
                    current_type,
                    argContext,
                    dc);
            }
            ...
            {
                ((ArrayAccessExpression)children[i]).reflectExpression(
                    current_type,
                    argContext,
                    dc);
            }
            ...
            if (i != children.length - 1 && children[i].isPrimitive())
            {
                throw new ReflectExpressionError(
                    " <Found "
                        + children[i]
                        + " which has primitive, not reference type in reference expression.>",
                    this);
            }
            ...
            if (!(current_type instanceof Class) && children[i].isReference())
            {
                System.err.println(
