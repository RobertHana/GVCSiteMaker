...
public class NameAnalysis {
	...
	try {
		...
		while (outerclazz.lastIndexOf('$') != -1) {
		    outerclazz = outerclazz.substring
			(0, outerclazz.lastIndexOf('$'));
		...
		for (int i = 0; i < importedTypes.length; i++) {
		    if (importedTypes[i].endsWith("." + classname)) {
	...
	System.out.println("Namecache hits (%) : " 
			   + ((double) hit) / ((double) (hit + nohit)));
    } 
	...
	if (classpath_lookup.containsKey(classname)) {
	    ...
	    File[] fs = getSourceFilesForClassName(classname);
	    for (int i = 0; i < fs.length; i++) {
		// It is a source file
		try {
	...
	boolean classNameWasFound 
	    = classVisitor.containsClassName(name);
